...
struct LinearSymbolicComparator : BaseComparator<LinearSymbolicComparator> {
  ...
  void init(PtrMatrix<int64_t> A, EmptyMatrix<int64_t> = EmptyMatrix<int64_t>{},
    ...
    V.resizeForOverwrite(toRow(numVar + numCon), toCol(2 * numCon));
    V = 0;
    V(0, 0) = pos0;
    // V = [A' 0
    //      S  I]
  ...
  void initNonNegative(PtrMatrix<int64_t> A, size_t numNonNegative) {
    ...
    numVar = size_t(A.numCol());
    V.resizeForOverwrite(toRow(numVar + numConTotal), toCol(2 * numConTotal));
    V = 0;
    V(0, 0) = 1;
    // B = [ A_0 A_1
    //        0   I  ]
  ...
  void initNonNegative(PtrMatrix<int64_t> A, PtrMatrix<int64_t> E,
    ...
    numVar = size_t(A.numCol());
    V.resizeForOverwrite(toRow(numVar + numInEqConTotal),
                         toCol(2 * numInEqConTotal + numEqCon));
    V = 0;
    V(0, 0) = 1;
    // B = [ A_0 A_1
    //        0   I  ]
  ...
  void init(PtrMatrix<int64_t> A, PtrMatrix<int64_t> E, bool pos0 = true) {
    ...
    numVar = size_t(A.numCol());
    const size_t numEqCon = size_t(E.numRow());
    V.resizeForOverwrite(toRow(numVar + numInEqCon),
                         toCol(2 * numInEqCon + numEqCon));
    V = 0;
    // V = [A' E' 0
    //      S  0  I]
    V(0, 0) = pos0;
  ...
  void initCore() {
    auto &A = V;
    Row<> R = V.numRow();
    U.resizeForOverwrite(R, toCol(*R));
    U = 0;
  ...
  auto isEmpty() -> bool {
    ...
    // Column rank deficient case
      ...
      // where y2 = y2+ - y2-
      IntMatrix expandW(toRow(numSlack), NSdim * 2 + 1);
      ...
      IntMatrix Wcouple{toRow(0), expandW.numCol()};
  ...
  [[nodiscard]] auto greaterEqual(PtrVector<int64_t> query) const -> bool {
    ...
    // Column rank deficient case
      ...
      Col<> NSdim = V.numCol() - numRowTrunc;
      // expand W stores [c -JV2 JV2]
      //  we use simplex to solve [-JV2 JV2][y2+ y2-]' <= JV1D^(-1)Uq
      // where y2 = y2+ - y2-
      IntMatrix expandW(toRow(numSlack), NSdim * 2 + 1);
